#include "include.h"
float q[4]; /* 四元数*/
float gyrof[3];  /* X/Y/Z 三轴角加速度*/            
unsigned char dmpdatas[42];
extern float Angle_of_Target;
...
/* 计算Yaw、Pitch、Roll */
float Yaw_Offset = 0;
extern char Uart0_Recv_Flag;   //UART0 接收数据标志位
extern struct SAngle stcAngle;
extern float Yaw,Yaw_Ref;    //MPU9250 采样到的偏航角 以及串口陀螺仪采样到的参考偏航角
extern int16 Rear_x ,Rear_y ,Token_x , Token_y, Head_x, Head_y;
extern int16 Blue_pos_x1,Blue_pos_y1,Blue_pos_x2,Blue_pos_y2;
extern int16 Sys_Count;
char Curve_120 = 0,Curve_240 = 0;   // 1 表示顺时针经过60° 300°  -1 表示表示逆时针经过60° 300° 0 表示未经过 
void getyawpitchroll(void)
{   
    getquaternion();//计算四元数
    Yaw =-atan2(2.0*(q[0]*q[3] + q[1]*q[2]),1 - 2.0*(q[2]*q[2] + q[3]*q[3]))*57.3;
      if(gpio_get(KEY_E) == 0)
      {
  ...
  //  Yaw = Yaw - Yaw_Offset;   
    ...
    if( Yaw >180 )  Yaw = Yaw - 360; else if( Yaw <-180 ) Yaw = Yaw +360;
    if( Yaw > 115 && Yaw < 135)     //经过  120°
    {
      
      if(Curve_240 == -1)  Curve_120 = -1;
      else if(Curve_240 == 1) Curve_240 = 0;   
      else if(Curve_240 == 0) Curve_120 = 1;
    }
    else if( Yaw > -135 && Yaw < -115)
    {
      if(Curve_120 == 1) Curve_240 = 1;
      else if(Curve_120 == -1)  Curve_120 = 0;
      else if(Curve_120 == 0) Curve_240 = -1;
    }
   ...
   /*—————————一旦经过 0 °附近 两个标志位都清零—————————-*/
    if( -10 < Yaw && Yaw < 10)
    {
      if(Curve_120 == 1 && Curve_240 == 1)
      {
        Yaw_Offset = Yaw_Offset +4.5;
  //      LED_P8x16Str(4,4,"here");
      }
      else if(Curve_120 == -1 && Curve_240 == -1)
      {
        Yaw_Offset = Yaw_Offset -4.5;
      }
      Curve_120 = 0;
...
void MPU9250_search()
{
  ...
  fifo_count = MPU9250_getFIFOCount();//读取FIFO计数
        ...
        {           
            if(fifo_count >= 0x2a)//如果FIFO值 >42,此时DMP转换完成并且数值正常
            {				
